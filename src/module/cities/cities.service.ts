...
async  create(createCityDto: CreateCityDto): Promise<{message: string}> {
    ...
    const foundedCity = await this.cityRepo.findOne({where: {name: enumCityName}})

    if(foundedCity) throw new BadRequestException("This City already exists")
...
async  findAllCities(): Promise<City[]> {
  ...
  if(foundedCities.length === 0) throw new NotFoundException("No cities found")
